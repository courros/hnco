...
Population::sort()
{
  std::sort(_lookup.begin(), _lookup.end(), _comparison_index_value);
}
...
Population::partial_sort(int selection_size)
{
  ...
  std::partial_sort(_lookup.begin(), _lookup.begin() + selection_size, _lookup.end(), _comparison_index_value);
}
