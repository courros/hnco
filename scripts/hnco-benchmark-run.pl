...
sub iterate_runs
{
    ...
    if ($parallel) {
        foreach (1 .. $num_runs) {
            push @commands,
                "/usr/bin/time --quiet -f \"\%U\" -o $prefix/$_.time $cmd --print-results > $prefix/$_.out 2>> $prefix/$_.err";
        }
        print "added to the job queue";
    } else {
        foreach (1 .. $num_runs) {
            system("/usr/bin/time --quiet -f \"\%U\" -o $prefix/$_.time $cmd --print-results > $prefix/$_.out 2>> log.err");
            print ".";
        }
    }
}
